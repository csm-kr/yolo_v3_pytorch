...
from utils.label_info import voc_label_list, coco_label_list, voc_color_array, coco_color_array
...
def demo(opts, device, model):
    ...
    # 2. load .pth
    checkpoint = torch.load(f=os.path.join(opts.save_dir, opts.name, opts.name + '.{}.pth.tar'.
...
def visualize_detection_result(x, bbox, label, score):
    ...
    for j in range(len(bbox)):
        ...
        if opts.data_type == 'voc':
            label_list = voc_label_list
            color_array = voc_color_array

        elif opts.data_type == 'coco':
            label_list = coco_label_list
...
if __name__ == '__main__':
    ...
    parser = configargparse.ArgumentParser('yolo v3 demo', parents=[get_args_parser()])
